...
// ===== V4.3.3 — Configurações e idiomas (isolado do cálculo) =====
(function(){
  ...
  const I18N={
    "pt-BR":{title:"Meu Salário Japão 🇯🇵",subtitle:"Calculadora e controle de jornada",settings:"⚙️ Configurações",language:"🌐 Idioma",note:"O idioma é salvo neste aparelho. Seus registros não são alterados.",close:"Fechar",theme:"Alternar tema"},
    "ja-JP":{title:"日本給与 🇯🇵",subtitle:"給与計算と勤務時間管理",settings:"⚙️ 設定",language:"🌐 言語",note:"言語設定はこの端末に保存されます。記録データは変更されません。",close:"閉じる",theme:"テーマ切替"},
    "en-US":{title:"Japan Salary 🇯🇵",subtitle:"Salary calculator and work-time tracker",settings:"⚙️ Settings",language:"🌐 Language",note:"Your language is saved on this device. Your records are not changed.",close:"Close",theme:"Toggle theme"}
  };
  let lang=localStorage.getItem("msj-language")||"pt-BR";
  if(!I18N[lang]) lang="pt-BR";

  function open(){settingsModal.classList.remove("hidden");}
  ...
  function apply(){
    ...
    if(h1) h1.textContent=x.title;
    if(sub) sub.textContent=x.subtitle;
    const title=document.getElementById("settingsTitle");
    const label=document.getElementById("languageLabel");
    const note=document.querySelector(".settingsNote");
    if(title) title.textContent=x.settings;
    if(label && label.firstChild) label.firstChild.nodeValue=x.language+"\n      ";
    if(note) note.textContent=x.note;
    settingsBtn.setAttribute("aria-label",x.settings);
    closeSettings.setAttribute("aria-label",x.close);
  ...
  languageSelect.addEventListener("change",function(){
    if(!I18N[languageSelect.value]){
      languageSelect.value=lang;
      alert("Este idioma será adicionado em uma próxima versão.");
      return;
    }
    lang=languageSelect.value;
    localStorage.setItem("msj-language",lang);
    apply();
  });
  apply();
})();
